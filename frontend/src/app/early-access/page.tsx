...
export default function EarlyAccessPage() {
  ...
  /** Hash the handle for a stable 0-15 rarity jitter. Shown live in
   *  the rarity meter so the preview matches the final roll. */
  ...
  function handleSignals(): XSignals {
    return {
      handle,
      displayName: handle,
      ...
      // Mock values until OAuth lands — these pad the score for
      // accounts that would likely qualify anyway.
      followerCount: 500,
      accountAgeDays: 800,
      followsBase: tasks.followBase,
      bioMentionsBase: false,
      baseTweetHits: tasks.replyPinned ? 1 : 0,
    };
  }

  async function startReveal() {
    const signals = handleSignals();
    const generated = generateCard(signals);

    // Bake task bonuses into the visible score so the meter's
    // prediction matches what we persist.
    let extra = 0;
    if (tasks.followAgentsCup) extra += 15;
    if (tasks.notificationsOn) extra += 10;
    // (+50 follow-base and +15 reply-pinned are already in
    //  computeRarityScore via the signals above.)
    if (extra > 0) {
      generated.score += extra;
      generated.signalBreakdown = [
        ...generated.signalBreakdown,
        ...(tasks.followAgentsCup
          ? [{ label: "Follows @agentscup", points: 15 }]
          : []),
        ...(tasks.notificationsOn ? [{ label: "Notifications on", points: 10 }] : []),
      ];
    }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 pt-10 pb-24">
        {phase === "landing" && <LandingHero onStart={() => setPhase("handle")} />}
